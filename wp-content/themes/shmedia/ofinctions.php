...
require_once(TEMPLATEPATH . '/include/core.php');
function truncate_excerpt($amount,$echo=true) {
...
function truncate_content($amount,$echo=true) {
    global $post, $shortname;
...
function upload_scripts() {
    ...
    wp_register_script('my-upload', get_bloginfo('template_directory').'/javascript/custom_uploader.js', array('jquery','media-upload','thickbox'));
...
function emm_paginate($args = null) {
    $defaults = array(
        ...
        'range' => 4, 'gap' => 1, 'anchor' => 1,
        'before' => '', 'after' => '',
        'title' => __(''),
        'nextpage' => __('Next','epress'), 'previouspage' => __('Previous','epress'),
        'echo' => 1
    );
    ...
    if ($pages > 1) {
        ...
        if ($page > 1 && !empty($previouspage)) {
            $output .= "<li class='deact'><a href='" . get_pagenum_link($page - 1) . "' class='prev-page'><<</a></li>";
        }
        ...
        if ($page < $pages && !empty($nextpage)) {
            $output .= "<li><a href='" . get_pagenum_link($page + 1) . "' '> >> </a></li>";
        }
...
function emm_paginate_loop($start, $max, $page = 0) {
    ...
    for ($i = $start; $i <= $max; $i++) {
        $output .= ($page === intval($i))
            ? "<li class='act'><a href='" . get_pagenum_link($i) . "' '>$i</a></li>"
